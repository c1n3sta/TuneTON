...
export default function MusicPlayer({ 
  ...
  const track = convertJamendoToTrack(currentJamendoTrack, currentTrack, currentTime, isLiked, isDisliked);
  ...
  useEffect(() => {
    const loadJamendoTracks = async () => {
      try {
        ...
        let trackToPlay = allTracks.find(track => track.name === currentTrack) || allTracks[0];
        if (trackToPlay) {
