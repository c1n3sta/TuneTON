-- Add indexes for unindexed foreign keys directly in Supabase SQL editor
...
-- Add index for any other potential tables with user_id foreign keys
CREATE INDEX IF NOT EXISTS idx_user_profiles_user_id ON public.user_profiles (user_id);
CREATE INDEX IF NOT EXISTS idx_user_settings_user_id ON public.user_settings (user_id);
CREATE INDEX IF NOT EXISTS idx_user_preferences_user_id ON public.user_preferences (user_id);

-- Output message to confirm completion
SELECT 'Indexes created successfully. Please refresh the database linter in the Supabase dashboard to verify the fixes.' AS result;