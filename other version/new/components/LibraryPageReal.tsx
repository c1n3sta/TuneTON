  ...
  const loadLibraryData = async () => {
    ...
    try {
      ...
      if (user?.access_token) {
        ...
        tuneTONAPI.setAccessToken(user.access_token);
      } else {
      ...
      const [playlistsData, likedTracksData, statsData] = await Promise.all([
        tuneTONAPI.getUserPlaylists().catch(err => {
          console.error('🔐 getUserPlaylists failed:', err);
          return [];
        }),
        tuneTONAPI.getLikedTracks().catch(err => {
          console.error('🔐 getLikedTracks failed:', err);
          return [];
        }),
        tuneTONAPI.getLibraryStats().catch(err => {
                      ...
                      >
                        <CardContent className="p-3 space-y-2">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground truncate">{playlist.tracks.length} tracks</p>
                            <p className="text-xs text-muted-foreground truncate">{getPlaylistDuration(playlist.tracks)}</p>
                          </div>
                        </CardContent>
                      </Card>
                    ))}
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-2 text-xs text-muted-foreground mt-1">
                                ...
                                <span>•</span>
                                <span>{getPlaylistDuration(playlist.tracks)}</span>
                                <span>•</span>
                                <span>{formatDateAdded(playlist.updatedAt)}</span>
                              </div>
                            </div>
