...
export default function MusicApp() {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (tg) {
        ...
        if (tg.initDataUnsafe?.user) {
          ...
          // Create a simple access token from Telegram user ID for development
          const mockAccessToken = `telegram_${telegramUser.id}_${Date.now()}`;
        ...
        const mockAccessToken = `dev_user_${Date.now()}`;
        const mockUser = {
          id: 12345,
          first_name: "Dev User",
          username: "devuser",
  ...
  const handleTelegramConnect = async () => {
    try {
      ...
      if (tg) {
        ...
        const mockAccessToken = `dev_fallback_${Date.now()}`;
        const mockUser = {
          id: 12345,
          first_name: "Dev User",
          username: "devuser",
  ...
  const handleTrackChange = (track: JamendoTrack | string) => {
    if (typeof track === 'string') {
      // Legacy support for string tracks - convert to mock JamendoTrack
      setCurrentTrack({
        id: 'legacy-' + Date.now(),
        name: track,
