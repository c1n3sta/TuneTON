...
export default function PlaylistDetailReal({ 
  ...
  const handleRemoveTrack = async (trackId: string) => {
    ...
    try {
      const updatedPlaylist = await tuneTONAPI.removeTrackFromPlaylist(playlist.id, trackId);
      if (updatedPlaylist) {
  ...
  const handleAddTrack = async (track: JamendoTrack) => {
    ...
    try {
      const updatedPlaylist = await tuneTONAPI.addTrackToPlaylist(playlist.id, track);
      if (updatedPlaylist) {
        setPlaylist(updatedPlaylist);
          ...
          <div className="flex flex-col items-center gap-4 px-6 mb-8">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground">
                {playlist.tracks.length} tracks • {getPlaylistDuration(playlist.tracks)}
              </p>
              ...
              <div className="flex items-center justify-center gap-2 mt-2">
                ...
                <span className="text-xs text-muted-foreground">
                  Updated {formatDateAdded(playlist.updatedAt)}
                </span>
              </div>
            </div>
          </div>
