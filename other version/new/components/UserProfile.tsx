  ...
  return (
    <div className="bg-[#0d1117] min-h-screen">
      <div className="max-w-md mx-auto bg-[#161b22] rounded-2xl overflow-y-auto pb-20">
        ...
        {/* Jamendo Integration */}
        <div className="px-6 mb-6">
          <h3 className="text-[#c9d1d9] text-base font-semibold mb-4">Music Integration</h3>
          <div className="space-y-3">
            <button 
              onClick={() => {
                // Import OAuth function dynamically
                import('../utils/jamendo-oauth').then(({ startJamendoOAuth, jamendoOAuth }) => {
                  if (jamendoOAuth.isAuthenticated()) {
                    console.log('Already connected to Jamendo');
                  } else {
                    startJamendoOAuth('profile_connect');
                  }
                });
              }}
              className="w-full bg-[#ff4400] text-[#dffcf8] py-3 px-4 rounded-lg text-sm font-medium flex items-center gap-2"
            >
              <TrendingUp className="w-4 h-4" />
              Connect Jamendo Account
            </button>
            <button 
              onClick={() => onNavigate?.("Home", "jamendo-test")}
              className="w-full bg-[#484f58] text-[#c9d1d9] py-3 px-4 rounded-lg text-sm font-medium flex items-center gap-2"
            >
