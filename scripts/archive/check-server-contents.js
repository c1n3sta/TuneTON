...
// Script to check the current contents of the server via FTP
...
async function listDirectoryRecursive(client, dirPath = '.', indent = '') {
  try {
    ...
    for (const file of files) {
      ...
      if (file.isDirectory) {
        await listDirectoryRecursive(client, `${dirPath}/${file.name}`, indent + '  ');
      } else {
        console.log(`${indent}  📄 ${file.name} (${file.size} bytes)`);
      }
    }
  } catch (error) {
    console.log(`${indent}❌ Error listing ${dirPath}: ${error.message}`);
  }
}
...
async function checkServerContents() {
  ...
  try {
    ...
    await listDirectoryRecursive(client, '.');
    
    // Check if backend directory exists
    try {
      console.log('\n🔍 Checking for backend directory...');
      await client.cd('backend');
      console.log('✅ Backend directory found!');
      console.log('\n📂 Backend directory contents:');
      await listDirectoryRecursive(client, '.');
      await client.cd('..'); // Go back to home directory
    } catch (error) {
      console.log('ℹ️  Backend directory not found or inaccessible');
    }
    
    // Check if tuneton directory exists
    try {
      console.log('\n🔍 Checking for tuneton directory...');
      await client.cd('tuneton');
      console.log('✅ Tuneton directory found!');
      console.log('\n📂 Tuneton directory contents:');
      await listDirectoryRecursive(client, '.');
      await client.cd('..'); // Go back to home directory
    } catch (error) {
  ...
  console.log('\n✅ Server contents check completed.');
}
