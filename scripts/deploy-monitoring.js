...
// Script to deploy the monitoring suite to the production server
...
console.log('Starting monitoring suite deployment...');
...
async function deploy() {
  ...
  try {
    ...
    // Check for required monitoring directory
    ...
    // Create monitoring directory on server if it doesn't exist
    try {
      await client.send('MKD monitoring');
      console.log('Created monitoring directory');
    } catch (error) {
      console.log('Monitoring directory already exists or error creating it');
    }
    
    // Change to monitoring directory
    await client.cd('monitoring');
    ...
    async function getFiles(dir, baseDir = '') {
      ...
      for (const item of items) {
        ...
        if (stat.isDirectory()) {
          ...
          try {
            await client.send(`MKD ${remoteDirPath}`);
            console.log(`Created directory: ${remoteDirPath}`);
          } catch (error) {
            console.log(`Directory ${remoteDirPath} already exists or error creating it`);
          }
    ...
    for (const file of filesToUpload) {
      try {
        ...
        if (!client.ftp.connected) {
          ...
          // Change to monitoring directory again
          await client.cd('monitoring');
        }
    ...
    if (failureCount === 0) {
      console.log('The monitoring suite has been uploaded to the production server.');
      console.log('Remember to run the setup scripts on the server to configure the monitoring tools.');
    } else {
