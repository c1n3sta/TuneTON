...
export default function ContestsPage({ onBack, onNavigate, onOpenContestDetail, onOpenRankingDetail, activeTab = "Contests" }: ContestsPageProps) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "Ending Soon": return "bg-[#d29922]";
      case "New": return "bg-[#2ea043]";
  ...
  const getDifficultyColor = (difficulty: string) => {
    switch (difficulty) {
      case "Beginner": return "bg-[#2ea043]";
      case "Intermediate": return "bg-[#d29922]";
      ...
      <div className="flex justify-center">
        <div className="w-[400px] bg-[#161b22] rounded-2xl min-h-screen relative overflow-hidden">
          ...
          <div className="flex items-center justify-between p-6">
            ...
            <h2 className="text-[18px] font-semibold text-[#c9d1d9]">Contests</h2>
          ...
          <div className="px-6 pb-32">
            ...
            <div className="mb-8">
              <h2 className="text-[18px] font-bold text-[#c9d1d9] mb-4">Featured Contest</h2>
              <div 
                onClick={() => onOpenContestDetail?.(featuredContest)}
                className="relative bg-[#484f58] rounded-2xl h-[200px] overflow-hidden w-full text-left hover:scale-[1.02] transition-transform cursor-pointer"
              >
                <div 
                ...
                <div className="absolute top-4 right-4">
                  <div className="bg-[#ff4400] px-3 py-1 rounded-full text-[12px] font-bold text-white">
                    FEATURED
                  </div>
                </div>
                <div className="absolute bottom-0 left-0 right-0 p-4">
                  <h3 className="text-[20px] font-bold text-white mb-1">{featuredContest.title}</h3>
                  <p className="text-[12px] text-[rgba(255,255,255,0.8)] mb-3">{featuredContest.description}</p>
                  
                  <div className="flex items-center gap-4 mb-4 text-[rgba(255,255,255,0.9)]">
                    <div className="flex items-center gap-1">
                      <Trophy className="w-4 h-4" />
                      <span className="text-[12px] font-bold">{featuredContest.prize}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <Clock className="w-4 h-4" />
                      <span className="text-[12px] font-bold">{featuredContest.timeLeft}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <Users className="w-4 h-4" />
                      <span className="text-[12px] font-bold">{featuredContest.participants}</span>
                    </div>
                  </div>
                  ...
                  <button 
                    ...
                    className={`w-full py-3 rounded-[7px] text-[14px] font-bold transition-colors ${
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-[16px] font-semibold text-[#c9d1d9]">Live Leaderboards</h3>
                <button
                  onClick={() => onNavigate?.("Ranking")}
                  className="text-[12px] text-[#ff22fb] hover:text-[#ff22fb]/80"
                >
                  View All Rankings
                </button>
              </div>
              ...
              <div className="bg-[#21262d] rounded-2xl p-4 border border-[#30363d]">
                <div className="flex items-center justify-between mb-3">
                  <h4 className="text-[14px] font-semibold text-[#c9d1d9]">Remix Rumble Leaderboard</h4>
                  <span className="text-[10px] bg-[#d29922] text-white px-2 py-1 rounded-full uppercase tracking-wide">Live</span>
                </div>
                  ...
                  {[
                    ...
                    <div key={index} className="flex items-center justify-between py-1">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <span className="text-[12px] font-semibold text-[#c9d1d9]">{entry.name}</span>
                          <p className="text-[10px] text-[#8b949e]">{entry.score}</p>
                        </div>
                      </div>
                      <button
                        onClick={() => onOpenRankingDetail?.(entry.name.toLowerCase(), 'remixers')}
                        className="text-[10px] bg-[#ff22fb] text-white px-2 py-1 rounded hover:bg-[#ff22fb]/90"
                      >
                        View
                      </button>
                    </div>
            ...
            <div className="mb-6">
              <h3 className="text-[16px] font-semibold text-[#c9d1d9] mb-3">Categories</h3>
              <div className="flex gap-2 overflow-x-auto pb-2">
                {categories.map((category) => (
                  ...
                  >
                    {category}
                    <span className="ml-2 bg-[#161b22] bg-opacity-50 px-2 py-0.5 rounded-full">
                      {category === "All" 
                        ? contests.length 
                        : contests.filter(c => c.category === category).length}
                    </span>
                  </button>
                ))}
              </div>
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-[16px] font-semibold text-[#c9d1d9]">Active Contests</h3>
                <span className="text-[12px] text-[#8b949e]">{filteredContests.length} contests</span>
              </div>
              ...
              <div className="space-y-4">
                {filteredContests.map((contest) => (
                  <div 
                    ...
                    className="bg-[#21262d] border border-[#30363d] rounded-2xl p-4 w-full text-left hover:bg-[#30363d] transition-colors cursor-pointer"
                  >
                      ...
                      <div className="flex-1 space-y-3">
                        <div>
                          <div className="flex items-center justify-between mb-1">
                            <h4 className="text-[16px] font-bold text-[#c9d1d9]">{contest.title}</h4>
                            <div className={`px-2 py-1 rounded-[5px] text-[10px] font-bold text-white uppercase tracking-[0.5px] ${getStatusColor(contest.status)}`}>
                              {contest.status}
                            </div>
                          </div>
                          <p className="text-[12px] text-[#8b949e] mb-2">{contest.description}</p>
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <span className={`px-2 py-1 rounded-[5px] text-[10px] font-bold uppercase tracking-[0.5px] text-white ${getDifficultyColor(contest.difficulty)}`}>
                              {contest.difficulty}
                            </span>
                            <span className="text-[10px] text-[#8b949e] bg-[#161b22] px-2 py-1 rounded-[5px]">
                          ...
                          <button
                            ...
                            className={`px-4 py-2 rounded-[7px] text-[12px] font-bold transition-colors ${
            ...
            <div className="bg-[#21262d] rounded-2xl p-4 mb-6">
              <h3 className="text-[16px] font-semibold text-[#c9d1d9] mb-4">Your Contest Stats</h3>
              <div className="grid grid-cols-3 gap-4">
                <div className="text-center">
                  <div className="text-[20px] font-bold text-[#ff22fb]">
                    {joinedContests.size}
                  </div>
                  <div className="text-[12px] text-[#8b949e]">Joined</div>
                </div>
                <div className="text-center">
                  <div className="text-[20px] font-bold text-[#2ea043]">2</div>
                  <div className="text-[12px] text-[#8b949e]">Won</div>
                </div>
                <div className="text-center">
                  <div className="text-[20px] font-bold text-[#d29922]">5</div>
                  <div className="text-[12px] text-[#8b949e]">Top 10</div>
                </div>
              </div>
            </div>
            ...
            {/* Quick Actions */}
            <div className="mb-6">
              <h3 className="text-[16px] font-semibold text-[#c9d1d9] mb-3">Quick Actions</h3>
              <div className="grid grid-cols-2 gap-3">
                <button className="bg-[#21262d] hover:bg-[#30363d] p-4 rounded-xl transition-colors flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <div className="text-[14px] font-semibold text-[#c9d1d9]">My Submissions</div>
                    <div className="text-[12px] text-[#8b949e]">View & manage</div>
                  </div>
                </button>
                ...
                <button className="bg-[#21262d] hover:bg-[#30363d] p-4 rounded-xl transition-colors flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <div className="text-[14px] font-semibold text-[#c9d1d9]">Schedule</div>
                    <div className="text-[12px] text-[#8b949e]">Upcoming events</div>
                  </div>
                </button>
              </div>
            </div>
          ...
          {/* Bottom Navigation */}
          <BottomNavigation activeTab="Contests" onTabChange={onNavigate || (() => {})} />
        </div>
      </div>
    </div>
  );
